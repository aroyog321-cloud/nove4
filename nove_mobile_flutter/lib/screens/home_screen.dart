...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  final _SortOrder _sortOrder = _SortOrder.updatedDesc;
  ...
  Map<String, int> _getCategoryCounts(List<Note> notes) {
    ...
    for (final cat in _categories) {
      if (cat == 'All') {
        ...
        counts[cat] = notes.where((n) => (n.category ?? '').toLowerCase() == cat.toLowerCase()).length;
      }
    }
    return counts;
  }
              ...
              children: [
                ...
                if (counts[cat] != null && counts[cat]! > 0) ...[
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(color: active ? Colors.white.withValues(alpha: 0.2) : NoveColors.accent(context).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(NoveRadii.full)),
      ...
      itemBuilder: (ctx, i) {
        if (i == 0) {
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: active ? NoveColors.terracotta.withValues(alpha: 0.1) : Colors.transparent, 
                borderRadius: BorderRadius.circular(NoveRadii.full), 
        ...
        return GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), 
            decoration: BoxDecoration(
              color: active ? NoveColors.amber.withValues(alpha: 0.15) : Colors.transparent, 
              borderRadius: BorderRadius.circular(NoveRadii.full), 
              border: Border.all(color: active ? NoveColors.amberDark : NoveColors.cardBorder(context), width: 1),
              boxShadow: active ? [BoxShadow(color: NoveColors.amber.withValues(alpha: 0.2), blurRadius: 10, offset: const Offset(0, 4))] : null,
            ), 
